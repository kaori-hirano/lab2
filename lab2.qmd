...
```{r p2q2}
??count
(count(p, year)) # need to group by year? not sure what it wants us to do with count
# NOT DONE

length(p$employer_id) - length(unique(p$employer_id)) # subtracts all employers from dulplicates removed employers list length
```
...
## Q4
```{r p2q4}
#  Does there seem to be a relationship between the size of a company and the pay gap?
# Use at least one visualization to answer this question and explain how you arrived at
# your conclusion.
```
